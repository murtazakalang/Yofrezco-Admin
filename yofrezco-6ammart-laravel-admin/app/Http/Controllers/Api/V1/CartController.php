...
class CartController extends Controller
{
    public function get_carts(Request $request)
    {
        ...
        // Remove orphaned cart items (where the product has been deleted)
        $orphanedCarts = $carts->filter(function ($cart) {
            return $cart->item === null;
        });
        foreach ($orphanedCarts as $orphanedCart) {
            $orphanedCart->delete();
        }

        // Filter out orphaned items and format the remaining ones
